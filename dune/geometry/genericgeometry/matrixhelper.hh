  ...
  namespace GenericGeometry
  {
    ...
    struct MatrixHelper
    {
                  ...
                  typename Traits :: template Matrix< n, m > :: type &ret )
      {
        ...
        if( (n == 2) && (m == 2) )
        {
          // code generated by maple, copied from fmatrix.hh
          const FieldType det = (A[0][0]*A[1][1] - A[0][1]*A[1][0]);
          const FieldType det_1 = 1.0/det;
          ret[0][0] =   A[1][1] * det_1;
          ret[0][1] = - A[0][1] * det_1;
          ret[1][0] = - A[1][0] * det_1;
          ret[1][1] =   A[0][0] * det_1;
          ...
          const FieldType x1 = A[0][1] * A[1][2];
          const FieldType x2 = A[0][0] * A[1][2];
          const FieldType x3 = A[0][0] * A[1][1];
          const FieldType x4 = A[0][2] * A[1][1];
          const FieldType x5 = A[1][0] * A[0][2];
          const FieldType x6 = A[0][1] * A[1][0];
          const FieldType x6m3 = x6 - x3;
          const FieldType x5m2 = x5 - x2;
          const FieldType x4m1 = x4 - x1;
          const FieldType detsqr = x6m3 * x6m3 + x5m2 * x5m2 + x4m1 * x4m1;

          ret[0][0] = -x6m3 * A[1][1] - x5m2 * A[1][2];
          ret[0][1] = x6m3 * A[0][1] + x5m2 * A[0][2];
          ret[1][0] = x6m3 * A[1][0] - x4m1 * A[1][2];
          ret[1][1] = -x6m3 * A[0][0] + x4m1 * A[0][2];
          ret[2][0] = x5m2 * A[1][0] + x4m1 * A[1][1];
          ret[2][1] = -x5m2 * A[0][0] - x4m1 * A[0][1];
          ret /= detsqr;

          return sqrt(detsqr);
        } else if ((n == 3) && (m == 3)) {
          // code generated by maple, copied from fmatrix.hh
          const FieldType t4  = A[0][0] * A[1][1];
          const FieldType t6  = A[0][0] * A[1][2];
          const FieldType t8  = A[0][1] * A[1][0];
          const FieldType t10 = A[0][2] * A[1][0];
          const FieldType t12 = A[0][1] * A[2][0];
          const FieldType t14 = A[0][2] * A[2][0];

          const FieldType det = (t4*A[2][2]-t6*A[2][1]-t8*A[2][2]+
                   t10*A[2][1]+t12*A[1][2]-t14*A[1][1]);
          const FieldType t17 = 1.0/det;

          ret[0][0] =  (A[1][1] * A[2][2] - A[1][2] * A[2][1])*t17;
          ret[0][1] = -(A[0][1] * A[2][2] - A[0][2] * A[2][1])*t17;
          ret[0][2] =  (A[0][1] * A[1][2] - A[0][2] * A[1][1])*t17;
          ret[1][0] = -(A[1][0] * A[2][2] - A[1][2] * A[2][0])*t17;
          ret[1][1] =  (A[0][0] * A[2][2] - t14) * t17;
          ret[1][2] = -(t6-t10) * t17;
          ret[2][0] =  (A[1][0] * A[2][1] - A[1][1] * A[2][0]) * t17;
          ret[2][1] = -(A[0][0] * A[2][1] - t12) * t17;
          ret[2][2] =  (t4-t8) * t17;
          return abs(det);
        } else {
          typename Traits :: template Matrix< m , m > :: type aat;
