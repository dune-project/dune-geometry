    ...
    struct MatrixHelper
    {
                  ...
                  typename Traits :: template Matrix< n, m > :: type &ret )
      {
        ...
        if( (n == 2) && (m == 2) )
        {
          ...
          // based on code generated by maple via
          // CodeGeneration[C](LinearAlgebra[MatrixInverse](
          //   Matrix(2,3,symbol=A), method=pseudo), resultname=ret, optimize);
          FieldType t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12;
          t1 = A[0][1] * A[1][1];
          t2 = A[0][2] * A[1][2];
          t3 = -t1 - t2;
          t4 = A[1][2] * A[1][2];
          ...
          t11 = -2;
          const FieldType det =
              (t11 * (t2 * t1 - t7 * A[0][0]) + t10 * t5 + t4 * t8 + t6 * t9);
          const FieldType det_1 = 1 / det;
          t12 = A[0][0] * A[1][0];
          t2 = -t12 - t2;
          t1 = -t12 - t1;
          ret[0][0] = (t6 * A[0][0] + t7) * det_1;
          ret[0][1] = -(-t3 * A[0][0] + (-t8 - t10) * A[1][0]) * det_1;
          ret[1][0] = (t2 * A[1][1] + t4 * A[0][1]) * det_1;
          ret[1][1] = (t2 * A[0][1] + (t9 + t10) * A[1][1]) * det_1;
          ret[2][0] = (t1 * A[1][2] + t5 * A[0][2]) * det_1;
          ret[2][1] = (t1 * A[0][2] + (t8 + t9) * A[1][2]) * det_1;
          return det;
        } else if ((n == 3) && (m == 3)) {
          // code generated by maple, copied from fmatrix.hh
